...
const recuperationListeDesAliments = document.querySelector(".listeDesAliments")! as HTMLTableElement;
const recuperationTypesAliments = document.querySelector(".typesAliments")! as HTMLSelectElement;

afficherAliments("Tous")

function genererBaliseAliments(aliment:Aliments) : string{
    return `
    <tr>
        <td>${aliment.nom}</td>
...
function afficherAliments(selectedValue: string): void {
    ...
    for (let aliment of Aliments.listeAliments) {
        ...
        if (
            (selectedValue === "Mauvais" && aliment.qualiteNutrinionnelle === ClassQqualiteNutrinionnelle.MAUVAIS) ||
